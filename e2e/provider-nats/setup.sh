...
# AUTH account has jetstream enabled so the NatsPolicyProvider can access KV
cat > nats-server.conf <<EOF
server_name: nauts-test-provider-nats
jetstream {
  store_dir: /tmp/nauts-test-provider-nats-jetstream
    max_mem: 128M
    max_file: 1G
}
...
accounts {
    ...
    # JetStream enabled so the policy provider can use KV buckets
    AUTH {
        jetstream: enabled
        users: [
            { nkey: $AUTH_USER_PUB }
