...
| Package | `policy` | `jwt` | `identity` | `provider` | `auth` |
|---------|:--------:|:-----:|:----------:|:----------:|:------:|
| `policy` | — | — | — | — | — |
| `jwt` | ✓ | — | — | — | — |
| `identity` | — | — | — | — | — |
...
`policy` and `identity` are standalone. `jwt` depends only on `policy` for the `NatsPermissions` type. `provider` depends on `policy` (types) and `jwt` (Signer). `auth` is the top-level orchestrator.
